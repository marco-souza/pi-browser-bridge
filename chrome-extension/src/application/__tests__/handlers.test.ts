...
describe("handleScreenshot", () => {
	...
	test("returns error for invalid format", async () => {
		const result = await handleScreenshot("req-1", { format: "gif" }, fakeDeps);
	...
	test("returns error for restricted URL", async () => {
		...
		const result = await handleScreenshot("req-3", { format: "png" }, fakeDeps);
		expect(result.error).toBeDefined();
	...
	test("calls captureVisibleTab for valid params and unrestricted URL", async () => {
		...
		const result = await handleScreenshot("req-4", { format: "png" }, fakeDeps);
	...
	test("returns error on capture failure", async () => {
		...
		const result = await handleScreenshot("req-7", { format: "png" }, fakeDeps);
		expect(result.error).toBeDefined();
		expect(result.error?.code).toBe("RESTRICTED_URL");
	});
	...
	test("returns generic error for unknown capture failure", async () => {
		fakeDeps.getActiveTabUrl.mockResolvedValue("https://example.com");
		fakeDeps.captureVisibleTab.mockRejectedValue(new Error("Something broke"));

		const result = await handleScreenshot("req-8", { format: "png" }, fakeDeps);
		expect(result.error).toBeDefined();
		expect(result.error?.code).toBe("UNKNOWN_ACTION");
	...
	test("returns fullPage warning when requested", async () => {
		fakeDeps.getActiveTabUrl.mockResolvedValue("https://example.com");
		fakeDeps.captureVisibleTab.mockResolvedValue("data:image/png;base64,full");
	...
	test("accepts null params gracefully (uses defaults)", async () => {
		fakeDeps.getActiveTabUrl.mockResolvedValue("https://example.com");
		fakeDeps.captureVisibleTab.mockResolvedValue("data:image/png;base64,def");
	...
	test("returns UNKNOWN_ACTION when tabId is provided", async () => {
		const result = await handleScreenshot("req-15", { tabId: 123 }, fakeDeps);
		expect(result.error).toBeDefined();
		expect(result.error?.code).toBe("UNKNOWN_ACTION");
...
describe("handleWaitForElement", () => {
	...
	test("detects dynamically added element", async () => {
		...
		const promise = handleWaitForElement({
			selector: "#dynamic",
			timeout: 200,
		});
...
describe("handleWaitForText", () => {
	...
	test("scopes search to CSS selector", async () => {
		...
		const result2 = await handleWaitForText({
			text: "World",
			scope: "#a",
			timeout: 50,
		});
...
describe("Error propagation — domain errors bubble up correctly", () => {
	...
	test("handlers never throw — always return a result", async () => {
		...
		const handlers: Array<{
			...
			{
				name: "handleClick",
				fn: handleClick as (p: unknown) => Promise<unknown>,
				params: undefined,
			},
			{
				name: "handleType",
				fn: handleType as (p: unknown) => Promise<unknown>,
				params: undefined,
			},
			{
				name: "handleNavigate",
				fn: handleNavigate as (p: unknown) => Promise<unknown>,
				params: undefined,
			},
			{
				name: "handleRead",
				fn: handleRead as (p: unknown) => Promise<unknown>,
				params: undefined,
			},
			{
				name: "handleExec",
				fn: handleExec as (p: unknown) => Promise<unknown>,
				params: undefined,
			},
			{
				name: "handleWaitForElement",
				fn: handleWaitForElement as (p: unknown) => Promise<unknown>,
				params: undefined,
			},
			{
				name: "handleWaitForText",
				fn: handleWaitForText as (p: unknown) => Promise<unknown>,
				params: undefined,
			},
		];
