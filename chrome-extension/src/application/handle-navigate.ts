 ...
 */
function waitForLoadEvent(waitUntil: string, timeoutMs: number): Promise<void> {
	if (waitUntil === "load") {
 ...
 */
export async function handleNavigate(params: unknown): Promise<NavigateResult> {
	...
	if (isSamePage) {
		...
		return handleSamePageNavigation(targetUrl.hash || "", waitUntil, timeoutMs);
	}
