...
const bridgeToggle = document.getElementById("bridge-toggle") as HTMLInputElement | null;
const textarea = document.getElementById("allowlist") as HTMLTextAreaElement | null;
...
async function loadBridgeState(): Promise<void> {
  try {
    ...
    if (cfg) {
      const status = (cfg.connectionStatus as ConnectionStatus) ?? "disconnected";
...
async function saveAllowlist(): Promise<void> {
  ...
  if (patterns.length === 0) {
    setStatusMessage("Allowlist cannot be empty. Use * to allow all domains.", false);
    return;
  }
