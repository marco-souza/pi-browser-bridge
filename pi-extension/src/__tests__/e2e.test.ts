...
function createSimBrowser(
	...
	return (data: string) => {
		...
		switch (req.action) {
			case "navigate": {
				const url =
					typeof req.params?.url === "string" ? req.params.url : "about:blank";
				respond(req.id, {
					url,
			...
			case "screenshot": {
				const format = req.params?.format === "jpeg" ? "jpeg" : "png";
				respond(req.id, {
			...
			case "read": {
				const scope =
					typeof req.params?.selector === "string"
						? req.params.selector
						: "body";
				...
				const maxLength =
					typeof req.params?.maxLength === "number"
						? req.params.maxLength
						: 50000;
...
describe("E2E: Full round-trips", () => {
	...
	describe("browser_wait_for_element", () => {
		test("returns element info when found", async () => {
			...
			const result = await browserWaitForElement({
		...
		test("handles connection loss during wait", async () => {
			...
			const promise = browserWaitForElement({
				selector: "#something",
				timeout: 5000,
			});
			await sleep(100);
	...
	describe("browser_wait_for_text", () => {
		...
		test("respects scope selector", async () => {
			...
			const result = await browserWaitForText({
				text: "Section 1",
				scope: "h2",
			});
			expectSuccess(result);
		...
		test("all tools reject when no client is connected", async () => {
			...
			for (const r of results) {
				...
				if (r.status === "fulfilled") {
					...
					expect(r.value.content[0].text).toMatch(
						/not connected|BROWSER_NOT_CONNECTED|No browser/i,
					);
				}
			}
		});
