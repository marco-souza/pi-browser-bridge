	...
	test("error response with suggestion", () => {
		const res: Response<"type"> = {
			...
			error: {
				...
				suggestion:
					"Use a selector targeting an input, textarea, or contenteditable element",
			},
		};
	...
	test("TAB_NOT_FOUND error for type action on closed tab", () => {
		const res: Response<"type"> = {
			...
			error: {
				...
				suggestion:
					"Use listTabs to find a valid tab, or createTab to open a new one.",
			},
		};
		expect(res.error!.code).toBe("TAB_NOT_FOUND");
	...
	test("TAB_NOT_FOUND error for closeTab on already-closed tab", () => {
		const res: Response<"closeTab"> = {
			...
			error: {
				...
				suggestion:
					"The tab may have been closed by another action. List tabs to find valid tabIds.",
			},
		};
		expect(res.error!.code).toBe("TAB_NOT_FOUND");
		expect(res.result).toBeUndefined();
